...
using Bravillian.Generators.SqlGen.Pipeline;
using Bravillian.Generators.SqlGen.Pipeline._1_Ingestion;
using Bravillian.Generators.SqlGen.Pipeline._2_SchemaRefinement;
using Bravillian.Generators.SqlGen.Pipeline._3_CSharpTransformation;
using Bravillian.Generators.SqlGen.Pipeline._4_CodeGeneration;
using Xunit;

namespace Bravillian.Generators.Tests.SqlGenerator.EndToEnd;
...
public class EndToEndTests
{
    ...
    public void GenerateComplex_FromSql_ShouldProduceCorrectCSharp()
    {
        ...
        var sql = """
            CREATE TABLE [erp].[TaxCode] (
                ...
                CONSTRAINT [PK_erp_TaxCode] PRIMARY KEY ([BravillianTenantId] ASC, [TaxCodeId] ASC),
                CONSTRAINT [FK_erp_TaxCode_BravillianTenantId] FOREIGN KEY ([BravillianTenantId]) REFERENCES [pw].[BravillianTenant] (
                    [BravillianTenantId]
                )
            )
            GO

            CREATE UNIQUE INDEX [UQ_erp_TaxCode_BravillianTenantId_ErpId]
                ON [erp].[TaxCode] ([BravillianTenantId], [ErpId]);
            GO
            ...
            /*
               ...
               TaxCode table only has FK to BravillianTenant which is covered by the PK.
            */
            ...
            */
            CREATE NONCLUSTERED INDEX [IX_TaxCode_BravillianTenantId_TaxCodeId_Covering]
                ON [erp].[TaxCode] ([BravillianTenantId], [TaxCodeId])
        ...
        var config = new SqlConfiguration
        {
            ...
            Namespace = "Bravillian.Database.PwSql",
            ...
            GlobalTypeMappings = [
                new()
                {
                    Match = new()
                    {
                        ColumnNameRegex = "BravillianTenantId",
                    },
                    Apply = new()
                    {
                        CSharpType = "BravillianTenantIdentifier",
                    },
                },
                new()
                {
    ...
    {
        var configJson = File.ReadAllText(@"C:\src\internal\src\modules\Database\Bravillian.Database.PwSql.Model\sql.generator.config.json");
        var sqlFiles = Directory.GetFiles(@"C:\src\internal\src\modules\Database\Bravillian.Database.PwSql\", "*.sql", SearchOption.AllDirectories)
    ...
    public void GenerateFull_FromSingleSql_ShouldProduceCorrectCSharp()
    {
        var configJson = File.ReadAllText(@"C:\src\internal\src\modules\Database\Bravillian.Database.PwSql.Model\sql.generator.config.json");
        string[] sqlFiles = [
            File.ReadAllText(@"C:\src\internal\src\modules\Database\Bravillian.Database.Audit\events\Tables\AuditEntry.sql"),
            //File.ReadAllText(@"C:\src\internal\src\modules\Database\Bravillian.Database.PwSql\erp\Tables\ApInvoice.sql"),
            //File.ReadAllText(@"C:\src\internal\src\modules\Database\Bravillian.Database.PwSql\erp\Views\ApInvoiceDetail.sql"),
        ];
