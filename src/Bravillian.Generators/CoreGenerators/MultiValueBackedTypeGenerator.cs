...
namespace Bravillian.Generators;
...
{
    public static GeneratorParams? GetParams(XElement xml, IPwLogger? logger)
    {
        ...
        var fields = xml.Elements()
            ...
            .ToList();
        return new(attributes.TryGetValue("name"), attributes.TryGetValue("namespace"), true, attributes.TryGetValue("separator"), attributes.TryGetValue("format"), attributes.TryGetValue("regex"), fields);
    }

    public static string? Generate(GeneratorParams? structToGenerate, IPwLogger? logger)
    {
    ...
    private static string GenerateTypedConstructor(in GeneratorParams relatedClass)
    {
        ...
        var ctorParameters = string.Join(", ", nonConstantFields.Select(f => $"{f.FieldType} {f.FieldName.ToLower()}"));
        ...
        {
            var ctorValue = "$\"" + string.Join("{Separator}", relatedClass.Fields.Select(f =>
            {
                if (f.ConstantValue != null)
                {
            ...
            return $$"""
    public const string Separator = "{{relatedClass.Separator ?? "|"}}";
...
""";
        ...
        else
        {
            ...
            return $$"""
    public const string RegexString = @"{{regexPattern}}";
    ...
    private static string GenerateTryParse(in GeneratorParams relatedClass)
    {
        ...
        if (useSeparator && !hasSerializedNames)
        {
            ...
            var tryParse = hasNonStringFields
                ? string.Join("\r\n                  && ", nonConstantFields.Where(f => !f.IsString).Select((f, i) => $"{f.FieldType}.TryParse(split[{i}], out {f.FieldType} {f.FieldName.ToLower()})"))
                : null;
        ...
        else
        {
            ...
            foreach (var field in relatedClass.Fields)
            {
                ...
                else
                {
                    ...
                    {
                        parseVars.Add($"            var {field.FieldName.ToLower()} = {field.FieldType}.Parse(match.Groups[\"{field.FieldName.ToLower()}\"].Value);");
                    }
                }
            }
...
""";
    ...
    {
        var properties = string.Join("\r\n\r\n", relatedClass.Fields.Select(f => $"    public {f.FieldType} {f.FieldName} {{ get; }}"));
...
public readonly partial record struct {{relatedClass.Name}}
        : IComparable,
          ...
          Bravillian.IHasValueConverter,
          Bravillian.IPwParsable<{{relatedClass.Name}}>,
          Bravillian.IStringBackedType<{{relatedClass.Name}}>,
          Bravillian.IMultiBackedType<{{relatedClass.Name}}, {{fieldTypes}}>
{
...
""";
    ...
    public readonly record struct GeneratorParams
    {
        ...
        public GeneratorParams(string name, string ns, bool isPublic, string separator, string format, string regex, IReadOnlyList<FieldInfo> fields)
        {
            this.Name = name;
            this.Namespace = ns;
    ...
    public readonly record struct FieldInfo
    {
        ...
        public FieldInfo(string fieldName, string fieldType, string? serializedName = null, string? constantValue = null, string? constantTypeValue = null)
        {
