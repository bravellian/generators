...
public readonly partial record struct {{relatedClass.Name}}
        : IComparable,
          ...
          Bravillian.IPwEnum<{{relatedClass.Name}}>
{
{{constValues}}
...
{{allValuesLine}}
    ...
    /// Throws <see cref="Bravillian.UnhandledEnumValueException"/> if no match is found.
    /// </summary>
    /// <param name="{{string.Join(", ", relatedClass.EnumValues.Select(p => $"case{p.Name}"))}}">The delegates to execute for each case.</param>
    /// <exception cref="Bravillian.UnhandledEnumValueException">Thrown when the current value is not handled by any case.</exception>
...
{{matchCases}}
            default:
                throw new Bravillian.UnhandledEnumValueException(this);
        }
    }
